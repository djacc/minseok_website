// Auto-generated gallery data
window.galleryData = [
  {
    subfolder: "1_superellipse",
    images: [
      {
        path: "web_images/1_superellipse/01_process.png",
        title: "process"
      },
      {
        path: "web_images/1_superellipse/02_eurostile_comparison.jpg",
        title: "eurostile_comparison"
      },
      {
        path: "web_images/1_superellipse/04_eurostile_comparison.jpg",
        title: "eurostile_comparison"
      },
      {
        path: "web_images/1_superellipse/05_omega_speedmaster_tv.jpg",
        title: "omega_speedmaster_tv"
      },
      {
        path: "web_images/1_superellipse/06_minuscule_o.jpg",
        title: "minuscule_o"
      },
      {
        path: "web_images/1_superellipse/07_basic_latin.png",
        title: "basic_latin"
      },
      {
        path: "web_images/1_superellipse/08_specimen.png",
        title: "specimen"
      },
      {
        path: "web_images/1_superellipse/09_application.jpg",
        title: "application"
      }
    ]
  },
  {
    subfolder: "2_saveroom",
    images: [
      {
        path: "web_images/2_saveroom/01_full_cover.png",
        title: "full_cover"
      },
      {
        path: "web_images/2_saveroom/02_record_shop.jpg",
        title: "record_shop"
      },
      {
        path: "web_images/2_saveroom/03_record_shop.jpg",
        title: "record_shop"
      }
    ]
  },
  {
    subfolder: "3_werkschau_plakat",
    images: [
      {
        path: "web_images/3_werkschau_plakat/01_südkreuz.jpg",
        title: "südkreuz"
      },
      {
        path: "web_images/3_werkschau_plakat/02_first_draft.jpg",
        title: "first_draft"
      },
      {
        path: "web_images/3_werkschau_plakat/03_final_draft.jpg",
        title: "final_draft"
      }
    ]
  },
  {
    subfolder: "4_loehr_salonediasschau",
    images: [
      {
        path: "web_imaegs_hidden/4_loehr_salonediasschau/01_jazz_seat_detail.jpeg",
        title: "jazz_seat_detail"
      },
      {
        path: "web_imaegs_hidden/4_loehr_salonediasschau/02_jazz.jpeg",
        title: "jazz"
      },
      {
        path: "web_imaegs_hidden/4_loehr_salonediasschau/03_jazz_salone_di_aschau.jpeg",
        title: "jazz_salone_di_aschau"
      },
      {
        path: "web_imaegs_hidden/4_loehr_salonediasschau/04_jazz_salone_di_aschau.jpeg",
        title: "jazz_salone_di_aschau"
      }
    ]
  },
  {
    subfolder: "5_asuzuge",
    images: [
      {
        path: "web_images/5_asuzuge/01_cover.jpg",
        title: "cover"
      },
      {
        path: "web_images/5_asuzuge/02_table_of_contents.jpg",
        title: "table_of_contents"
      },
      {
        path: "web_images/5_asuzuge/03_flyleaf.jpg",
        title: "flyleaf"
      },
      {
        path: "web_images/5_asuzuge/04_pages_18-19.jpg",
        title: "pages_18-19"
      },
      {
        path: "web_images/5_asuzuge/05_pages_20-21.jpg",
        title: "pages_20-21"
      },
      {
        path: "web_images/5_asuzuge/06_pages_38-39.jpg",
        title: "pages_38-39"
      },
      {
        path: "web_images/5_asuzuge/07_pages_38-39.jpg",
        title: "pages_38-39"
      }
    ]
  }
];
